...
- [ ] 사용자는 설비를 선택하고 센서 데이터를 수기 입력하거나 CSV로 업로드할 수 있어야 한다.
- [ ] 시스템은 NASA C-MAPSS 기반 이상 감지 모델로 이상 점수와 이상 여부를 계산해야 한다.
- [ ] 시스템은 OpenAI API를 사용해 이상 판단 근거를 현장 언어로 자연어 설명해야 한다.
- [ ] 시스템은 저장된 과거 분석 이력 중 유사 사례 3건을 검색해 보여줘야 한다.
- [ ] 사용자는 분석 결과에 대해 실제 조치 내용, 담당자, 결과 메모를 기록할 수 있어야 한다.
- [ ] 대시보드는 설비별 최근 이상 이력, 조치 현황, 기본 정확도 요약을 표시해야 한다.
- [ ] AI허브 제조 데이터는 제품 메인 플로우가 아니라 보조 검증 데이터셋으로 사용해야 한다.
- [ ] 첫 사용 시 빈 상태에서는 온보딩 안내와 샘플 분석 1건을 제공해야 한다.
...
- NASA C-MAPSS 기준으로 MVP에서 사용할 센서 필드와 라벨 정의 확정
- AI허브 데이터는 보조 검증 지표 산출용 최소 범위로 축소
- 화면 단위 정의
  - 데이터 입력 / 업로드 화면
  - 분석 결과 화면
  - 조치 기록 화면
  - 대시보드 화면
- 데이터 모델과 API 스펙을 기준으로 구현 이슈 분해
- 구현 후 검증 항목 고정
  - 이상 감지 정확도
  - 사용자 플로우 완주 여부
  - 근거 설명 품질 샘플
  - NASA vs AI허브 보조 검증 비교
